...
const createOnRouteHandler = ({
  ...
  return function fastifyQueryOnRoute (routeOptions) {
    ...
    if (routeOptions[kFastifyQueryRoute] ||
        ...
        this.hasRoute({ method: 'QUERY', url })) { return }
...
async function fastifyQuery (fastify, opts) {
  const { addQueryTypes, decorateReply, exposeQueryRoutes, overrideQueryTypes, strict } = (opts = {
    ...
    filterReply: ({ statusCode }) => statusCode >= 200 && statusCode < 300,
    filterRequest: true,
  ...
  Object.entries(queryTypes).forEach(([contentType, queryFn]) => {
    if (typeof queryFn !== 'function') { throw new TypeError(`Query for content type ${contentType} must be a function`) }
